...
def test_align_columns_orders_by_config(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    shuffled = frame[["target", "comp_active", "feature_02", "feature_01"]]
    aligned = align_columns(shuffled, column_cfg)
    assert list(aligned.columns) == ["feature_01", "feature_02", "comp_active", "target"]


def test_align_columns_does_not_mutate_caller(
    frame: pd.DataFrame, column_cfg: ColumnConfig
) -> None:
    lenient = replace(column_cfg, strict_schema=False)
...
def test_strict_schema_rejects_missing_column(
    frame: pd.DataFrame, column_cfg: ColumnConfig
) -> None:
    with pytest.raises(SchemaError, match="feature_02"):
        align_columns(frame.drop(columns=["feature_02"]), column_cfg)


def test_lenient_schema_fills_missing_column(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    lenient = replace(column_cfg, strict_schema=False)
    aligned = align_columns(frame.drop(columns=["feature_02"]), lenient)
    assert (aligned["feature_02"] == 0.0).all()


def test_resolve_schema_splits_on_dtype(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    frame["comp_active"] = frame["comp_active"].astype(str)
    schema = resolve_schema(frame, column_cfg)
...
def test_resolve_schema_honours_forced_categoricals(
    ...
    cfg = replace(column_cfg, categorical_columns=("comp_active",))
    schema = resolve_schema(frame, cfg)
    assert schema.categorical == ("comp_active",), "numeric dtype must not override the config"


def test_transform_standardises_features(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
...
def test_test_data_reuses_train_statistics(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
...
def test_unknown_category_is_encoded_as_zeros(
    frame: pd.DataFrame, column_cfg: ColumnConfig
) -> None:
    cfg = replace(column_cfg, categorical_columns=("comp_active",))
    pre = Preprocessor(cfg).fit(frame)
...
def test_inverse_target_round_trips(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    pre = Preprocessor(column_cfg).fit(frame)
    _, y = pre.transform(frame)
    np.testing.assert_allclose(pre.inverse_target(y), frame["target"].to_numpy(), atol=1e-6)


def test_numeric_nan_filled_with_mean(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    holed = frame.copy()
    holed.loc[0, "feature_01"] = np.nan
    x, _ = Preprocessor(column_cfg).fit_transform(holed)
    assert np.isfinite(x).all()


def test_index_of_uses_expanded_feature_names(
    frame: pd.DataFrame, column_cfg: ColumnConfig
) -> None:
    """comp_active sits at config index 2, but after one-hot the layout shifts."""
    cfg = replace(column_cfg, categorical_columns=("feature_02",))
...
def test_transform_before_fit_raises(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    with pytest.raises(NotFittedError):
        Preprocessor(column_cfg).transform(frame)


def test_fit_requires_target_column(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    with pytest.raises(SchemaError, match="required to fit"):
        Preprocessor(column_cfg).fit(frame.drop(columns=["target"]))


def test_fitted_state_exposes_the_estimators(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    state = Preprocessor(column_cfg).fit(frame).fitted_state
    assert state.scaler is not None
    assert state.output_scaler is not None
    assert state.onehot is not None


def test_fitted_state_carries_no_schema(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
...
def test_fitted_state_before_fit_raises(column_cfg: ColumnConfig) -> None:
    with pytest.raises(NotFittedError):
        _ = Preprocessor(column_cfg).fitted_state


def test_restore_reproduces_transforms(frame: pd.DataFrame, column_cfg: ColumnConfig) -> None:
    pre = Preprocessor(column_cfg).fit(frame)
    restored = Preprocessor.restore(column_cfg, pre.fitted_state, pre.schema, pre.feature_names)
